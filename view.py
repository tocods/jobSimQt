...
from qdarktheme.qtpy.QtWidgets import QGraphicsView, QGraphicsPathItem, QGraphicsPixmapItem
from qdarktheme.qtpy.QtCore import Qt, QRect
from qdarktheme.qtpy.QtGui import QPainter
...
class GraphicView(QGraphicsView):
    ...
    # 在画布上添加主机
    def createGraphicHostItem(self, host_name, host_type, img, width, height, onlyCpu=False, Host_class=globaldata.Host):
    ...
    # 在画布上添加交换机
    def createGraphicSwitchItem(self, switch_name, switch_type, img, width, height, Switch_class=globaldata.Switch):
        item = SwitchGraphicItem(switch_name, switch_type, img, width, height, Switch_class=Switch_class)
        item.setPos(0, 0)
        self.gr_scene.add_node(item)
        return item
        ...
        if event.button() == Qt.MouseButton.RightButton:
            ...
            # 交换机
                ...
                if isinstance(self.mouse_pos_item.switchAttr, TsnSwitch):
                    self.editSwitchNetargsWindowTsn.setSwitchGraphicItem(self.mouse_pos_item)
                    self.editSwitchNetargsWindowTsn.show()
                else:
                    self.editSwitchNetargsWindowUdp.setSwitchGraphicItem(self.mouse_pos_item)
