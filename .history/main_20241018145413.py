...
class JobSimQt(QMainWindow):
    ...
    def saveToProject(self):
        ...
        with open(path, "w") as f:
            json.dump([host.toDict() for host in sysSim.hosts.values()], f)
        path = project.projectPath + "/jobs.json"
        with open(path, "w") as f:
            json.dump([job.toDict() for job in sysSim.jobs.values()], f)
        path = project.projectPath + "/faults.json"
        with open(path, "w") as f:
            json.dump([fault.toDict() for fault in sysSim.faults.values()], f)
    ...
    def _run(self):
        ...
        if sysSim.jobs == {}:
            ...
            self._ui.central_window.centralWidget().setEnabled(True)
            return
        execute = "java -jar ./jobSim/gpuworkflowsim.jar " + project.projectPath + "/OutputFiles " + project.projectPath + "/hosts.json " + project.projectPath + "/jobs.json " + project.projectPath + "/faults.json " + str(0) + " " + str(100)
        print(execute)
