...
class JobSimQt(QMainWindow):
    ...
    def _initResult(self):
        ...
        for jobRecord in self.job_results:
            seeMore = QPushButton()
            seeMore.setText("查看")
            seeMore.clicked.connect(partial(self._initJobChartView, jobRecord.jobName))
            total = 0
            suc = 0
            timeout = 0
            for jobRun in jobRecord.jobRuns:
                ...
                if jobRun.status == "Success":
                    ...
                    timeout += 1
            self.jobTable.setItem(i, 0, QTableWidgetItem(jobRecord))
    ...
    def _initJobChartView(self, jobRecord):
        ...
        for jobRun in jobRecord.jobRuns:
            ...
            self.jobRunTable.setItem(i, 1, QTableWidgetItem(jobRun.hostName))
            self.jobRunTable.setItem(i, 2, QTableWidgetItem(jobRun.startTime))
            self.jobRunTable.setItem(i, 3, QTableWidgetItem(jobRun.endTime))
            self.jobRunTable.setItem(i, 4, QTableWidgetItem(jobRun.status))
            i += 1
