...
class JobSimQt(QMainWindow):
    ...
    def _initKernelTable(self, kernel_num, gpu_task):
        ...
        item5 = QTableWidgetItem("需求显存")    
        item5.setBackground(QColor(192, 192, 192))
        self.jobInfoPage.gputable.setItem(i, 4, item5)
        item6 = QTableWidgetItem("输入数据大小(MB)")
        item6.setBackground(QColor(192, 192, 192))
        self.jobInfoPage.gputable.setItem(i, 5, item6)
        item7 = QTableWidgetItem("输出数据大小(MB)")
    ...
    def _run(self):
        ...
        hosts = sysSim.hosts.values()
        jobs = sysSim.jobs.values()
        faults = sysSim.faults.values()
        hosts_json = json.dumps([hosts.__dict__ for host in sysSim.hosts.values()], indent=4, default=lambda o: o.__dict__)
        with open(projectPath + "/hosts.json", 'w') as write_f:
            write_f.write(hosts_json)
        jobs_json = json.dumps([job.__dict__ for job in sysSim.jobs], indent=4, default=lambda o: o.__dict__)
        with open(projectPath + "/jobs.json",'w') as write_f:
            write_f.write(jobs_json)
        fault_json = json.dumps([fault.__dict__ for fault in sysSim.faults], indent=4, default=lambda o: o.__dict__)
        with open(projectPath + "/faults.json", 'w') as write_f:
        ...
        if jobs == []:
            ...
            return
        execute = "java -jar ./jobSim/gpuworkflowsim.jar " + projectPath + "/OutputFiles " + projectPath + "/jobSim/input/hosts.json " + projectPath + "/jobSim/input/jobs.json " + projectPath + "/jobSim/input/faults.json " + str(0) + " " + str(100)
        print(execute)
