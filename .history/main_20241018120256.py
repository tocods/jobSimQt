...
class JobSimQt(QMainWindow):
    ...
    def _applyJob(self):
        ...
        if self.kernel_num > 0:
            self.nowJob.gpu_task = GPUTaskInfo(self.jobInfoPage.gputable.item(1, 4).text(), self.jobInfoPage.gputable.item(1, 5).text(), self.jobInfoPage.gputable.item(1, 6).text())
            kernels = []
            for i in range(self.jobInfoPage.gputable.rowCount() - 1):
                kernel = KernelInfo(int(self.jobInfoPage.gputable.item(i + 1, 1).text()), (int)(self.jobInfoPage.gputable.item(i + 1, 2).text()), (int)(self.jobInfoPage.gputable.item(i+ 1, 3).text()), (int)(self.jobInfoPage.gputable.item(i+1, 4).text()), (int)(self.jobInfoPage.gputable.item(i+1, 5).text()), (int)(self.jobInfoPage.gputable.item(i+1, 6).text()))
                kernels.append(kernel)
            self.nowJob.gpu_task.kernels = kernels
