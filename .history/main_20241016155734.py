...
class JobSimQt(QMainWindow):
    ...
    def _initHostInfo(self, host: HostInfo, ifTrueHost=True):
        ...
        if len(host.gpu_infos):
            self.hostInfoPage.gpuName.setText(host.gpu_infos[0].name)
            self.hostInfoPage.gpuRam.setValue(host.gpu_infos[0].ram)
            self.hostInfoPage.gpuCoreNum.setValue(host.gpu_infos[0].cores)
            self.hostInfoPage.gpuFlops.setText(str(host.gpu_infos[0].mips))
