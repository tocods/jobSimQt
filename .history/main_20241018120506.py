...
class JobSimQt(QMainWindow):
    ...
    def _applyJob(self):
        ...
        if self.kernel_num > 0:
            ...
            for i in range(self.jobInfoPage.gputable.rowCount() - 1):
                kernel = GPUTaskInfo.KernelInfo(int(self.jobInfoPage.gputable.item(i + 1, 1).text()), int(self.jobInfoPage.gputable.item(i + 1, 2).text()), int(self.jobInfoPage.gputable.item(i + 1, 3).text()), int(self.jobInfoPage.gputable.item(i + 1, 5).text()), int(self.jobInfoPage.gputable.item(i + 1, 6).text()))
