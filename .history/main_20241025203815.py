...
class JobSimQt(QMainWindow):
    ...
    def _initFaultInfo(self, fault: FaultGenerator, ifTrueFault=True):
        if ifTrueFault:
            ...
            if fault.mttf_type == "Normal":
                ...
                self.faultInfoPage.show.setChart(self.__getLogNormalLine(fault.mttf_scale, fault.mttr_scale))
                self.faultInfoPage.show2.setChart(self.__getLogNormalLine(fault.mttr_scale, fault.mttr_scale))
            elif fault.mttf_type == "Weibull":
                self.__getWeiBullLine(fault.mttf_scale, fault.mttr_scale)
            elif fault.mttf_type == "Gamma":
                self.__getGaussianLine(fault.mttf_scale, fault.mttr_scale)
        else:
            self.nowFault = None
            self.faultInfoPage.aim.clear()
            #self.faultInfoPage.aim.addItems(sysSim.hosts.keys())
    ...
    def __getLogNormalLine(self, avg, avg2):
        ...
        data2 = rng2.lognormal(avg2, 1, 200000) 
    ...
    def __getWeiBullLine(self, avg, avg2):
        ...
        rng2 = np.random.default_rng()
        data2 = rng2.weibull(avg2, 200000) 
        res_freq2 = stats.relfreq(data2, numbins=100)
        pdf_value2 = res_freq2.frequency
        x2 = res_freq2.lowerlimit + np.linspace(0, res_freq2.binsize * res_freq2.frequency.size, res_freq2.frequency.size)
        self.series2=QLineSeries()
    ...
    def __getGaussianLine(self, avg, avg2):
        print("get gaussian line")
        self.chart = QChart()
        rng = np.random.default_rng()
        ...
        rng2 = np.random.default_rng()
        data2 = rng2.gamma(avg2, 1, 200000) 
        res_freq2 = stats.relfreq(data2, numbins=100)
        pdf_value2 = res_freq2.frequency
        x2 = res_freq2.lowerlimit + np.linspace(0, res_freq2.binsize * res_freq2.frequency.size, res_freq2.frequency.size)
        self.series2=QLineSeries()
