...
class JobSimQt(QMainWindow):
    ...
    def _run(self):
        ...
        with open(projectPath + "/jobSim/input/faults.json", 'w') as write_f:
        ...
        if sysSim.jobs == []:
            ...
            return
        execute = "java -jar ./jobSim/gpuworkflowsim.jar " + projectPath + "/OutputFiles " + projectPath + "/jobSim/input/hosts.json " + projectPath + "/jobSim/input/jobs.json " + projectPath + "/jobSim/input/faults.json " + str(sysSim.scheduler) + " " + str(sysSim.duration)
        print(execute)
        popen = subprocess.Popen(execute, shell=True, stdout=subprocess.PIPE,  universal_newlines=True, stderr=subprocess.STDOUT)
        out,err = popen.communicate()
        print('std_out: ' + out)
        #将日志信息显示在文本框中
        self.log.setText(out)
