...
class JobSimQt(QMainWindow):
    ...
    def _initResult(self):
        ...
        for jobRecord in self.job_results:
            ...
            suc = 0
            timeout = 0
            for jobRun in jobRecord.
            self.jobTable.setItem(i, 0, QTableWidgetItem(jobRecord.jobName))
            self.jobTable.setItem(i, 1, QTableWidgetItem(str(jobRecord.runTimes)))
            self.jobTable.setItem(i, 2, QTableWidgetItem(str(jobRecord.successTimes)))
            self.jobTable.setItem(i, 3, QTableWidgetItem(str(jobRecord.timeoutTimes)))
