...
class JobSimQt(QMainWindow):
    ...
    def _initResult(self):
        ...
        chart = self.painter.plotHostUtilization("udp_cpu", -1, float("inf"))
        self._ui.resultui.show.setChart(chart)
        chartCPU = self.painter.plotHostCPUUtilization("udp_cpu", -1, float("inf"))
        self._ui.resultui.showCPU.setChart(chartCPU)
        chartRam = self.painter.plotHostRamUtilization("udp_cpu", -1, float("inf"))
        self._ui.resultui.showRam.setChart(chartRam)
        chartGPU = self.painter.plotGpuUtilization("udp_cpu", -1, -1, float("inf"))
        self._ui.resultui.showGPU.setChart(chartGPU)

        # 填充表格
        host_num = len(self.cluster_result.hostRecords)
        self.hostTable = QTableWidget()
        self.hostTable.setColumnCount(3)
        self.hostTable.setRowCount(host_num)
        self.hostTable.setHorizontalHeaderLabels(["主机名", "CPU利用率", "内存利用率"])
