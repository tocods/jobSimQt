...
class JobSimQt(QMainWindow):
    ...
    def initGpuTable(self, gpu_num, gpu_infos):    
        ...
        for gpu_info in gpu_infos:
            ...
            del_gpu = QPushButton()
            del_gpu.setText("删除")
            del_gpu.clicked.connect(partial(self._delGpu, i))

            self.hostInfoPage.gputable.setCellWidget(i, 6, del_gpu)

    def _delGpu(self, row):
        print("del gpu")
        self.hostInfoPage.gputable.removeRow(row)
        self.nowHost.video_card_infos[0].gpu_infos.pop(row - 1)
        self.initGpuTable(len(self.nowHost.video_card_infos[0].gpu_infos), self.nowHost.video_card_infos[0].gpu_infos)
