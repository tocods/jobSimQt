...
class JobSimQt(QMainWindow):
    ...
    def _initResult(self):
        ...
        self.cluster = xmlParser.parseHostRecord()
        path = "project/output/jobRun.xml"
        xmlParser = XmlParser(path)
        self.jobresults = xmlParser.parseJobRecord()
        path = "project/output/faultRecords.xml"
        xmlParser = XmlParser(path)
        self.faultresults = xmlParser.parseFaultRecord()
        self.painter = Painter(self.cluster, self.jobs, self.faults)
