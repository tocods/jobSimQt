...
class JobSimQt(QMainWindow):
    def __init__(self, path) -> None:
        ...
        self._ui.resultui.layoutWidget.setGeometry(0, 0, screen_size.width() * 0.7, screen_size.height() * 0.7)
        self._initAll()
        self.initTreeView()
        self.setClicked()
        self.center()
    ...
    def _initHostInfo(self, host: HostInfo, ifTrueHost=True):
        ...
        if ifTrueHost:
            ...
            if os.path.exists(path):
                ...
                self.hostInfoPage.cpushow.setChart(chartCPU)
                chartRam = painter.plotHostRamUtilization(host.name, -1, float("inf"))
                self.hostInfoPage.ramshow.setChart(chartRam)
                chartGPU = painter.plotGpuUtilization(host.name, -1, -1, float("inf"))
                self.hostInfoPage.gpushow.setChart(chartGPU)
    ...
    def __initJobInfo(self, job, ifTrueJob=True):
        if ifTrueJob:
            ...
            self.nowJob = None
        if ifTrueJob:
            path = project.projectPath + "/OutputFiles/jobUtils.xml"
            if os.path.exists(path):
                xmlParser = XmlParser(path)
                self.job_result = xmlParser.parseJobRecord()
                painter = Painter([], self.job_result, [])
                chart = painter.plotJobUtilization(job.name, -1, float("inf"))
                self.jobInfoPage.show.setChart(chart)
