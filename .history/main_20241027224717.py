    ...
    def _initHostInfo(self, host: HostInfo, ifTrueHost=True):
        ...
        if ifTrueHost:
            ...
            if os.path.exists(path):
                ...
                chartCPU = self.painter.plotHostCPUUtilization(host.name, -1, float("inf"))
                self._ui.resultui.showCPU.setChart(chartCPU)
                chartRam = self.painter.plotHostRamUtilization(host.name, -1, float("inf"))
                self._ui.resultui.showRam.setChart(chartRam)
                chartGPU = self.painter.plotGpuUtilization(host.name, -1, -1, float("inf"))
