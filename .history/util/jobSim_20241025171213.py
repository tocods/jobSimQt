...
class CPUTaskInfo:
    def __init__(self, ram: Optional[any]=None, pes_number: Optional[int] = None, length: Optional[float] = None):
        self.ram = ram
...
class GPUTaskInfo:
    ...
    def getFLOPS(self) -> float:
        return sum([(float)(kernel.thread_length) * (int)(kernel.thread_num * kernel.block_num for kernel in self.kernels])
...
class TaskInfo:
    ...
    def getFLOPS(self) -> float:
        FLOPS = 0
        if self.cpu_task_info is not None:
            FLOPS += self.cpu_task_info.getFLOPS()
        if self.gpu_task_info is not None:
            FLOPS += self.gpu_task_info.getFLOPS()
        return FLOPS
...
class FaultGenerator:
    def __init__(self, type, scale: Optional[float]=None, shape: Optional[float]=None):
        ...
        self.mttf_shape = scale
        self.mttr_type = type
        self.mttr_scale = shape
...
def tranFromE2C(ename):
    if ename == "Normal":
        return "正态分布"
    elif ename == "LogNormal":
        return "对数正态分布"
    elif ename == "Weibull":
        return "韦伯分布"
    elif ename == "Gamma":
...
class ParseUtil:
    ...
    def parseJobs(self, path):
        ...
        for job in json_data:
            ...
            print(job["gpu_task"])
            if job["gpu_task"] is None:
                job_info.gpu_task = None
