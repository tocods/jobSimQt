...
class FaultGenerator:
    def __init__(self, type: Optional[int] = None, scale: Optional[float]=None, shape: Optional[float]=None):
        self.mttf_type = type
...
class jobSim:
    ...
    def addHostItem(self, item: GraphicItem, onlyCPU: bool):
        # 创建一个所有属性都为0的HostInfo对象
        self.hosts[item] = HostInfo("", [VideoCardInfo([])], [CPUInfo(0, 0)], 0)
        self.onlyCPU[item] = onlyCPU


    def bindHostInfo(self, item: GraphicItem, host_info: HostInfo):
        host_info.name = self.hosts[item].name
        self.hosts[item] = host_info
        host_info.print()

    def isHost(self, item: GraphicItem):
        return item in self.hosts
    
    def getHostInfo(self, item: GraphicItem):
