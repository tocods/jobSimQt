...
class JobSimQt(QMainWindow):
    ...
    def _getNormalLine(self, avg, avg2):
        ...
        data2 = rng.normal(avg2, 1, 200000) 
        res_freq2 = stats.relfreq(data2, numbins=100)
        pdf_value2 = res_freq2.frequency
        x2 = res_freq2.lowerlimit + np.linspace(0, res_freq2.binsize * res_freq2.frequency.size, res_freq.frequency.size)
        self.series=QLineSeries()
        self.series.setName("平均故障修复时间分布")
        # 填充QLineSeries
        for i in range(len(x)):
                self.series.append(x[i],pdf_value[i])
        self.chart.addSeries(self.series)
