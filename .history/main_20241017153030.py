...
from util.jobSim import sysSim, ParseUtil, HostInfo, CPUInfo, GPUInfo, VideoCardInfo, JobInfo, CPUTaskInfo, GPUTaskInfo, FaultGenerator
...
class JobSimQt(QMainWindow):
    ...
    def _applyFault(self):
        ...
        self.nowFault.aim = self.faultInfoPage.aim.currentText()
        self.nowFault.type = self.faultInfoPage.type.currentText()
        self.nowFault.time1 = self.faultInfoPage.time1.text()
        self.nowFault.time2 = self.faultInfoPage.time2.text()
        sysSim.faults.pop(name_before)
        sysSim.faults[self.nowFault.aim] = self.nowFault
        # 更新树
        for i in range(self.nowClect.childCount()):
            if self.nowClect.child(i).text(0) == name_before:
