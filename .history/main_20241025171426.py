...
class JobSimQt(QMainWindow):
    ...
    def _initResult(self):
        ...
        chart.setTitle("计算效率:" + str(percent_total * 100) + "%")
        chart.legend().hide()
        ...
        self.pieSeries = QPieSeries()
        self.pieSeries.append("平均任务运行时间:" + str(self.avergaeRunTime) + "s", self.avergaeRunTime)
        self.pieSeries.append(":" + str(self.duration - self.avergaeRunTime) + "s", self.duration - self.avergaeRunTime)
        self.pieSeries.setPieSize(0.5)

        chart = QChart()
        chart.addSeries(self.pieSeries)
        chart.setTitle("平均任务运行时间:" + str(self.avergaeRunTime) + "s")
        chart.legend().hide()
        ...
        self.throughput = getThroughput(self.job_results)
        self.throughput = round(self.throughput, 2)
        self.pieSeries = QPieSeries()
        self.pieSeries.append("吞吐率:" + str(self.throughput) + "FLOPS/s", self.throughput)
        self.pieSeries.append(":" + str(1 - self.throughput) + "FLOPS/s", 1 - self.throughput)
