...
from util.jobSim import sysSim, ParseUtil, HostInfo, CPUInfo, GPUInfo, JobInfo, CPUTaskInfo, GPUTaskInfo
...
class JobSimQt(QMainWindow):
    ...
    def _initHostInfo(self, host: HostInfo, ifTrueHost=True):
        ...
        if host.video_card_infos != []:
            self.hostInfoPage.gpuName.setText(host.video_card_infos[0].name)
            self.hostInfoPage.gpuRam.setValue(host.video_card_infos[0].ram)
            self.hostInfoPage.gpuCoreNum.setValue(host.video_card_infos[0].cores)
            self.hostInfoPage.gpuFlops.setText(str(host.video_card_infos[0].mips))
