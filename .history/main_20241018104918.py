...
class JobSimQt(QMainWindow):
    ...
    def _initResult(self):
        ...
        for faultRecord in self.fault_results:
            seeMore = QPushButton()
            seeMore.setText("查看")
            seeMore.clicked.connect(partial(self._initFaultInfo, faultRecord))
    ...
    def _initJobChartView(self, jobRecord):
        ...
        self._ui.resultui.jobTabs.setCurrentIndex(2)
        self.jobChart.setChart(chart)

    def _initFaultInfo(self, faultRecord):
        self.jobRunTable.setRowCount(0)
        self.faultMoreTable.setRowCount(1)
        if faultRecord.isFalseAlarm:
            faultRecord.falseAlarm = "是"
        self.faultMoreTable.setItem(0, 0, QTableWidgetItem(str(faultRecord.falseAlarm)))
        self.faultMoreTable.setItem(0, 1, QTableWidgetItem(str(faultRecord.reconstruct)))
        self.faultMoreTable.setItem(0, 2, QTableWidgetItem(str(faultRecord.reliabilityBefore)))
        self.faultMoreTable.setItem(0, 3, QTableWidgetItem(str(faultRecord.reliabilityAfter))

        chart = self.painter.plotFault(faultRecord.time)
        self._ui.resultui.faultTabs.setCurrentIndex(1)
        self._ui.resultui.faultChart.setChart(chart)
