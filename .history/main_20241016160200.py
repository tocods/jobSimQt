...
class JobSimQt(QMainWindow):
    ...
    def _initHostInfo(self, host: HostInfo, ifTrueHost=True):
        ...
        if host.video_card_infos != []:
            gpu_num = len(host.video_card_infos.gpu_infos)
            if gpu_num > 0:
                self.initGpuTable(gpu_num, host.video_card_infos.gpu_infos)
                
    def initGpuTable(self, gpu_num, gpu_infos):
        self.hostInfoPage.gpuTable.setRowCount(gpu_num)
        self.hostInfoPage.gputable.setHorizontalHeaderLabels(["GPU", "核心数", "频率", "内存", "带宽"])
