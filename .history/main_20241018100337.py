...
class JobSimQt(QMainWindow):
    ...
    def _initResult(self):
        ...
        self.hostTable.setColumnCount(4)
        self.hostTable.setRowCount(host_num)
        self.hostTable.setHorizontalHeaderLabels(["主机名", "CPU利用率(平均/最大)", "内存利用率(平均/最大)", "GPU利用率(平均/最大)"])
        for hostRecord in self.cluster_result.hostRecords:
            row = hostRecord.hostName
            cpu = f"{hostRecord.avgCpuUtilization:.2f}/{hostRecord.maxCpuUtilization[1]:.2f}"
            ram = f"{hostRecord.ramUtilization[0]:.2f}/{hostRecord.ramUtilization[1]:.2f}"
            gpu = f"{hostRecord.gpuUtilization[0]:.2f}/{hostRecord.gpuUtilization[1]:.2f}"
            self.hostTable.setItem(row, 0, QTableWidgetItem(hostRecord.hostName))
            self.hostTable.setItem(row, 1, QTableWidgetItem(cpu))
            self.hostTable.setItem(row, 2, QTableWidgetItem(ram))
            self.hostTable.setItem(row, 3, QTableWidgetItem(gpu)
            )
