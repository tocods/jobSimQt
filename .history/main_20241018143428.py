...
class JobSimQt(QMainWindow):
    ...
    def loadFromProject(self):
        file_name, _ = QFileDialog.getExistingDirectory(None, "Open File", "", "All Files (*);;Text Files (*.txt)")
        print(file_name)
    ...
    def _applyHost(self):
        ...
        if self.gpu_num > 0:
            ...
            for i in range(self.hostInfoPage.gputable.rowCount() - 1):
                ...
                videoCardInfo.pcie_bw = (int)(self.hostInfoPage.pcie.text()) 
    ...
    def initGpuTable(self, gpu_num, gpu_infos):    
        ...
        item6 = QTableWidgetItem("GPU 显存(GB)")
    ...
    def _run(self):
        ...
        if sysSim.jobs == {}:
            ...
            self._ui.central_window.centralWidget().setEnabled(True)
            return
        execute = "java -jar ./jobSim/gpuworkflowsim.jar " + projectPath + "/OutputFiles " + projectPath + "/hosts.json " + projectPath + "/jobs.json " + projectPath + "/faults.json " + str(0) + " " + str(100)
        print(execute)
    ...
    def _initResult(self):
        ...
        # 绘制折线图
        path = projectPath + "/OutputFiles/hostUtils.xml"
        xmlParser = XmlParser(path)
        self.cluster_result = xmlParser.parseHostRecord()
        path = projectPath + "/OutputFiles/jobRun.xml"
        xmlParser = XmlParser(path)
        self.job_results = xmlParser.parseJobRecord()
        path = projectPath + "/OutputFiles/faultRecords.xml"
        xmlParser = XmlParser(path)
